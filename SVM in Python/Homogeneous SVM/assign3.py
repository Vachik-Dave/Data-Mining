...
class DBReader:
	...
	def	find_quad_x(self):
		...
		for i in self.X:
			...
			temp = np.append(temp,root_2 * i[0]);		# root2 * x1
			temp = np.append(temp,root_2 * i[1]);		# root2 * x2
			temp = np.append(temp, root_2 * i[0] * i[1]);	# root2 * x1 * x2
			temp = np.append(temp, 1)			# 1 for bias
			if Quad_x.any():
	...
	def	cal_linear_kernel(self):
		...
		return	self.lin_K;		
	def	cal_hmogeneous_kernel(self):		
		Qud_K = np.zeros([self.size[0],self.size[0]]);
		for i in range(0,self.size[0]):
			for j in range(i,self.size[0]):
				Qud_K[i][j] = pow(self.lin_K[i][j],2);
				Qud_K[j][i] = Qud_K[i][j];
		return Qud_K;
...
def 	main():
	...
	try:
		...
		f_quad = open('quadratic-kernel.txt','w');
		f_equ = open('equations.txt','w');
	except IOError,IndexError:
...
#	InHo_Qud_K = reader.cal_inhmogeneous_kernel();
	...
	alpha = SVM_Grad_asc(X,Y,Linear_K,C,epsilon);
	for a in alpha:
		f_linear.write(repr(a) +'\n');
	num_sup = (alpha != 0.0).sum();
	f_linear.write('Number of support vectors: '+ repr(num_sup) +'\n');

	alpha1 = SVM_Grad_asc(Quad_X,Y,Homog_Qud_K,C,epsilon);
	for a in alpha1:
		f_quad.write(repr(a) +'\n');
	...
	W = find_W(alpha,X,Y);
	b = find_avg_b(alpha,W,X,Y,C);
	f_equ.write('Linear Kernel Equation:'+'\n'+repr(W[0])+' * X1');
	if W[1] >= 0:
		f_equ.write(' + '+repr(W[1])+' * X2');
	else:
		f_equ.write(' '+repr(W[1])+' * X2');
	if b[0][0] >=0:
		f_equ.write(' + '+repr(b[0][0])+' = 0\n');
	else:
		f_equ.write(' '+repr(b[0][0])+' = 0\n');

	W1 = find_W(alpha1,Quad_X,Y);				# weights are multiplied by root_2 values in Quad_X
	b1 = find_avg_b(alpha1,W1,Quad_X,Y,C);
	f_equ.write('\n'+'Homogeneous-Quadratic Kernel Equation:'+'\n'+repr(W1[0])+' * X1^2');
	if W1[1] >= 0:
		f_equ.write(' + '+repr(W1[1])+' * X2^2');
	else:
		f_equ.write(' '+repr(W1[1])+' * X2^2');
	if W1[2] >= 0:
		f_equ.write(' + '+repr(W1[2] * (2 ** 0.5))+' * X1');		#root 2 * X1	Original Equation
	else:
		f_equ.write(' '+repr(W1[2] * (2 ** 0.5))+' * X1');
	if W1[3] >= 0:
		f_equ.write(' + '+repr(W1[3] * (2 ** 0.5))+' *X2');		#root 2 * X2	Original Equation
	else:
		f_equ.write(' '+repr(W1[3] * (2 ** 0.5))+' *X2');
	if W1[4] >= 0:
		f_equ.write(' + '+repr(W1[4] * (2 ** 0.5))+' * X1*X2');		#root 2 * X1*X2	Original Equation
	else:
	...
	print 'Linear - Kernel Alpha - W - b values:';
	print alpha;
	print W;
	print b;

	print 'Quadratic Kernel Alpha - W - b values:';
	print alpha1;
	print W1;
	print b1;
